...
If the input complies with the required format, further checks will be performed on the keywords to check whether the keywords are valid.
This keywords checking is completed by the `processDepartmentKeywords` and `processPositionKeywords` method that are both within the `FilterCommandParser` class.
...
Code snippet of the `parse` method in `FilterCommandParser` calling the `processDepartmentKeywords` method:
...
If the keywords from the user are all valid, the departmentPredicate will be created accordingly and set to the `FilterCommand`.
