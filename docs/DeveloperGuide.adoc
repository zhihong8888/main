...
All logic commands comes with a dedicated parser. These parsers ensures that the input conforms to the following before
parsing to the actual command for execution:

include::UserGuide.adoc[tag=addScheduleCommandField]
...
.. Clear Schedules
.. Prerequisites: There must be at least 3 employees in the observable employee list panel
and completed `deleteLeaves` manual test above.
... Test Case: clearSchedules +
Expected: Schedule list has been cleared!

=== Undo
.. Completed `clearSchedules` manual test above.
... Test Case: undo +
Expected: Undo success!

=== Redo
.. Redo
.. Completed `undo` manual test above.
.. Test Case: redo +
