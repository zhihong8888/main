...
----
Afterwards, the parsers validate field input by creating a new `Type`, `Date`, `EmployeeId` and `Schedule`
object, where the object will `throw exception` if it does not conform to the field restrictions.
...
If the `Type` is equals to leave and work is not scheduled on the same date,
