...
* `VersionedAddressBook#undo()` -- Allows `VersionedAddressBook`, `VersionedScheduleList`, `VersionedExpensesList`
and `VersionedRecruitmentList` to restore the previous storage model state from its history.
* `VersionedAddressBook#redo()` --  Allows `VersionedAddressBook`, `VersionedScheduleList`, `VersionedExpensesList`
...
** Cons: More codes needed.
* **Alternative 2:**Have a single command to add work/leave schedule.
** Pros: Easier to code.
** Cons: Requires user to specify type of schedule.
