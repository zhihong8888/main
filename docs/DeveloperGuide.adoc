...
----
Afterwards, the parsers validate field input by creating a new `Type`, `Date` and `EmployeeId` object,
where the object will `throw exception` if it does not conform to the field restrictions.
...
It looks at the `Type` inside the `Schedule`.
