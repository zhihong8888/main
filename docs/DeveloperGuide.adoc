...
This checking of input compliance is facilitated by two helper methods, namely `isValidEmployeeExpensesId` from `EmployeeExpensesId` class and `isValidExpensesAmount` from `ExpensesAmount` class.

[NOTE]
If the input does not comply with the required format, an error message will be shown to the user.

Code snippet of `isValidEmployeeExpensesId` that shows the checking of the input compliance:
...
This checking of input compliance is facilitated by two helper methods, namely `areDepartmentKeywordsValid` and `arePositionKeywordsValid`, that is within the class.

[NOTE]
If the input does not comply with the required format, an error message will be shown to the user.

Code snippet of `FilterCommandParser` that shows the parsing of input and the checking of the input compliance:
...
Code snippet of `areDepartmentKeywordsValid` and `arePositionKeywordsValid` helper method:
...
Code snippet of `FilterCommand` that checks which prefix is present prior to updating `filteredPersonList`:
...
The result of the execution of `FilterCommand` is encapsulated as a `CommandResult` object which is returned to the `LogicManager`.
Upon the update of `filteredPersonList`, the program will display only employees that are of the specific department(s)
...
image::filterCommandSequenceDiagram.png[width="300"]
