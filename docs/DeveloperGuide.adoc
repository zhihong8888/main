...
No schedules will be created if a duplicate, existing work or leave is found on that date.
...
For each date specified by the user in the set of dates `for (Date date :setOfDates)`,
it will be checked by for each observable employee in the employee list `for (Person person : model.getFilteredPersonList())`
for the possibility of deleting work. Work schedule will be deleted if found on that date.
...
`CalculateLeaves` filters the schedule list to only schedules that
...
if the leaves and year matches the one to be calculated, it
