...
----
    public ModifyPayCommand parse(String args) throws ParseException {
        ...
        if (argMultimap.getValue(PREFIX_BONUS).isPresent()) {
            modSalaryDescriptor.setBonus(ParserUtil.parseBonus(
            argMultimap.getValue(PREFIX_BONUS).get()));
        }
        if (argMultimap.getValue(PREFIX_SALARY).isPresent()) {
            modSalaryDescriptor.setSalary(ParserUtil.parseSalary(
            argMultimap.getValue(PREFIX_SALARY).get()));
        }
...
----
    private static Person createModifiedPerson(Person personToEdit, ModSalaryDescriptor modSalaryDescriptor) {
        ...
        return new Person(updatedEmployeeId, updatedName, updatedDateOfBirth, updatedPhone, updatedEmail, updatedDepartment, updatedPosition, updatedAddress, updatedSalary, updatedBonus, updatedTags);
    }
----

The result of `ModifyPayCommand` is encapsulated as a `CommandResult` object which is passed back into the UI to reflect the newly modified Salary/Bonus.
...
** Cons: User may accidentally touched on sensitive data and ended up editting them
...
image::Schedule/schedule_class_diagram.png[width="650"]

==== Add Schedule
`addSchedule` command allows a new schedule to be added.
Shown below is the sequence diagram for adding a new schedule.

image::Schedule/Basic Sequence Diagram2.png[width="650"]

In addition, the Model checks whether the employee id exists before allowing addtion of new schedules.

Code snippet of `UniquePersonList.java` shows the checking of employee id:
...
* **Alternative 1 (current choice):** Save to a new XML storage file.
** Pros: Easier to scale into large number of schedules.
** Cons: More difficult to implement
* **Alternative 2:**Save into addresbook.xml storage file
** Pros: Easier to manage person and schedule dependencies.
** Cons: Scalability issues when schedule grows at a larger volume.
...
Expected: First employee's salary will be increased by 1000 if max salary value are not exceeded after modification.
Else, an error message "Salary should only contain numbers,
and it should not be blank. Only a maximum of 6 whole numbers and 2 decimal place are allowed.
(Max Salary store value is 999999.99)" will be shown.

.. Test case: modifyPay 1 s/-1000 +
Expected: First employee's salary will be decreased by 1000 if salary does not goes below min value after modification.
Else, an error message "Pay are not allowed to be zero or negative in value" will be shown.

.. Test case: modifyPay 1 s/%100 +
Expected: First employee's salary will be doubled if the salary does not exceed the max value after modification.
Else, an error message "Salary should only contain numbers, and it should not be blank.
Only a maximum of 6 whole numbers and 2 decimal place are allowed.
(Max Salary store value is 999999.99)" will be shown.

.. Test case: modifyPay 1 s/%-100 +
Expected: An error message "Pay are not allowed to be zero or negative in value" will be shown.

.. Test case: modifyPay 1 s/%-5 +
Expected: First employee's salary will be decreased by 5%.

.. Test case: modifyPay 1 b/2 +
Expected: First employee's bonus will be changed to 2 months of current bonus.

.. Test case: modifyPay 1 b/25 +
Expected: An error message "Bonus should only contain positive numbers and maximum of 2 decimal places from 0 to 24,
and it should not be blank" will be shown.

.. Test case: modifyPay 1 b/-1 +
Expected: Similar to previous.

.. Test case: modifyPay 1 s/1000 b/2 +
Expected: If max salary value are not exceeded after modification,
first employee's salary increased by 1000 and bonus value changed to 2 months of new salary.
Else, an error message "Salary should only contain numbers, and it should not be blank.
Only a maximum of 6 whole numbers and 2 decimal place are allowed.
(Max Salary store value is 999999.99)" will be shown.
...
the employee salary in the list exceed max salary value after modification.
Else, an error message "Salary should only contain numbers,
and it should not be blank. Only a maximum of 6 whole numbers and 2 decimal place are allowed.
(Max Salary store value is 999999.99)" will be shown.
...
if none of the employee salary goes below min value after modification.
Else, an error message "Pay are not allowed to be zero or negative in value" will be shown.

.. Test case: modifyAllPay s/%100 +
Expected: All the employees' salary shown in the list will be doubled
if none of the employee salary exceed the max value after modification.
Else, an error message "Salary should only contain numbers, and it should not be blank.
Only a maximum of 6 whole numbers and 2 decimal place are allowed.
(Max Salary store value is 999999.99)" will be shown.
...
Expected: All the employees' salary shown in the list will be decreased by 5%.

.. Test case: modifyAllPay b/2 +
Expected: All the employees' bonus shown in the list will be changed to 2 months of current bonus.

.. Test case: modifyAllPay b/25 +
Expected: An error message "Bonus should only contain positive numbers and maximum of 2 decimal places from 0 to 24,
and it should not be blank" will be shown.

.. Test case: modifyAllPay b/-1 +
Expected: Similar to previous.

.. Test case: modifyPay 1 s/1000 b/2 +
Expected: If max salary value are not exceeded for any employee shown in the list,
All the employees' salary shown in the list will be increased by 1000 and bonus value changed to 2 months of new salary.
Else, an error message "Salary should only contain numbers, and it should not be blank.
Only a maximum of 6 whole numbers and 2 decimal place are allowed.
(Max Salary store value is 999999.99)" will be shown.
// end::casper[]
