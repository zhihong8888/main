...
// tag::filter[]
=== Filter Feature
The command `filter` allows the user to filter employees based on their respective department and/or position within the company. The filtered list will then be displayed on the GUI in either ascending or descending name order based on the user's input.
