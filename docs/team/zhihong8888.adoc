...
`addSchedule`, `deleteSchedule`, `addWorks`, `deleteWorks`, `addLeaves`,  `deleteLeaves`, `calculateWorks`,
 `clearSchedules`,  `selectSchedule`.
** What it does: Allows the user to perform scheduling for the company.
** Justification: This feature improves the product significantly because HR admin requires to handle scheduling of the employees.
...
to keep save and keep track of which storage has been committed changes, which is essential for undo/redo
to work properly with multiple storage.
(https://github.com/CS2113-AY1819S1-T16-4/main/pull/123[#123]).
`list` command lists persons and schedules as well. `delete` command deletes person and all schedules linked to the person.
